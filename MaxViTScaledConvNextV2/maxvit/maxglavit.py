...
PRETRAINED_DICT = {
 "maxvit_scaled": {"imagenet": {224: "e5cfd6a6bd4dea939860b6d8a29a911a", 384: "25b074ca6258d526c4d931f887599fe1", 512: "c745836c38f0ea98fef30bb3186aaf17"}},
...
def MaxViT(
    ...
    nn = eca_module(inputs=nn, gamma=2.0, beta=1.0)
    ...
    for stack_id, (num_block, out_channel) in enumerate(zip(num_blocks, out_channels)):
        ...
        for block_id in range(num_block):
            ...
            global_block_id += 1
	    #In thıs line, convnext.block uses convnextv2 because use_grn parameter set as True
            nn = convnext.convnext.block(inputs=nn, output_channel=out_channel,use_grn=True, activation="gelu", name=name + "convnextv2/")
...
#@register_model
def MaxGlaViT(input_shape=(224, 224, 3), num_classes=1000, drop_connect_rate=0, classifier_activation="softmax", pretrained="imagenet", **kwargs):
    num_blocks = [2, 2, 2, 2]
