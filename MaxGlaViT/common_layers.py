...
def channel_attention(inputs, name=None):
    """Channel Attention Module for CBAM without gamma and beta."""
    channel_axis = -1
    filters = inputs.shape[channel_axis]
    ...
    avg_pool = layers.Conv2D(filters // 16, kernel_size=1, padding='same', use_bias=False)(avg_pool)
    avg_pool = layers.Activation('relu')(avg_pool)
    avg_pool = layers.Conv2D(filters, kernel_size=1, padding='same', use_bias=False)(avg_pool)
    avg_pool = layers.Activation('sigmoid')(avg_pool)

    # Global Max Pooling
    max_pool = layers.GlobalMaxPooling2D()(inputs)
    max_pool = layers.Reshape((1, 1, filters))(max_pool)
    max_pool = layers.Conv2D(filters // 16, kernel_size=1, padding='same', use_bias=False)(max_pool)
    max_pool = layers.Activation('relu')(max_pool)
    max_pool = layers.Conv2D(filters, kernel_size=1, padding='same', use_bias=False)(max_pool)
    max_pool = layers.Activation('sigmoid')(max_pool)
...
def spatial_attention(inputs, name=None):
    """Spatial Attention Module for CBAM."""
    # Convolution along the channel dimension to produce a 1x1 feature map
    ...
    # 7x7 Convolution
    concat = layers.Conv2D(1, kernel_size=7, strides=1, padding='same', use_bias=False)(concat)
    concat = layers.Activation('sigmoid')(concat)
    
    # Spatial Attention
    spatial_attention = layers.Multiply()([inputs, concat])
    
    return spatial_attention

def cbam_block(inputs, name=None):
    print("cbamdan selamlar")
    """Convolutional Block Attention Module (CBAM) without gamma and beta."""
    x = channel_attention(inputs, name=name)
    x = spatial_attention(x, name=name)
    return x
