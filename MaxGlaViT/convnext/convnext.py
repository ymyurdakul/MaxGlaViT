...
def block1(inputs, output_channel, layer_scale_init_value=1e-6, use_grn=False, layer_norm_epsilon=1e-6, drop_rate=0, activation="gelu", name=""):
...
def block(inputs, output_channel, layer_scale_init_value=1e-6, use_grn=False, layer_norm_epsilon=1e-6, drop_rate=0, activation="gelu", name=""):
    input_channel = inputs.shape[-1]  # Get the number of input channels
    nn = depthwise_conv2d_no_bias(inputs, kernel_size=7, padding="same", strides=1, use_bias=True, name=name)
    nn = layer_norm(nn, epsilon=layer_norm_epsilon, name=name)
    nn = nn if backend.image_data_format() == "channels_last" else layers.Permute((2, 3, 1), name=name + "permute_pre")(nn)
    nn = layers.Dense(4 * output_channel, name=name + "up_dense")(nn)
    nn = activation_by_name(nn, activation, name=name)
    if use_grn:
        nn = global_response_normalize(nn, axis=-1, name=name + "grn_")  # Handle channels last or first
    nn = layers.Dense(output_channel, name=name + "down_dense")(nn)
    nn = nn if backend.image_data_format() == "channels_last" else layers.Permute((3, 1, 2), name=name + "permute_post")(nn)
    if input_channel != output_channel:
        shortcut = layers.Conv2D(output_channel, kernel_size=1, strides=1, padding="same", name=name + "shortcut_conv")(inputs)
    else:
        shortcut = inputs
    return add_with_layer_scale_and_drop_block(shortcut, nn, layer_scale=layer_scale_init_value, drop_rate=drop_rate, name=name)
